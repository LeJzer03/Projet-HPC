...
#SBATCH --cpus-per-task=4         # Number of OpenMP threads (adjust as needed)
#SBATCH --time=20:00
#SBATCH --output=shallow_openmp_%j.out
...
# Compile the C file with gcc and OpenMP support (and with all the flags sugested by the professor)
echo "Compiling ${SOURCE_FILE}..."
gcc -march=native -Wall -Wextra -Ofast -fopenmp -o ${EXEC_FILE} ${SOURCE_FILE} -lm #-fopt-info for an optimization report #-O3 for a more general optimization (-0fast is agressive)
if [ $? -ne 0 ]; then
...
# Execute the simulation with OpenMP
${EXEC_FILE} ${PARAM_FILE}
...
mv ${SLURM_SUBMIT_DIR}/shallow_openmp_${SLURM_JOB_ID}.out ${DATA_OUTPUT_DIR}/
